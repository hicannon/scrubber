...
def genSVMTrainInstance(im, bounds, windowSize=10, returnAll = False, mask = None):
	...
	for y in range(sy/windowSize):
		...
		for x in range(sx/windowSize):
			...
			maxMean = [max(a,b) for (a,b) in zip(meanDiff, maxMean)]
			modeDiff = [abs(a-b) for (a,b) in zip(wMode, mode)]
			maxMode = [max(a,b) for (a,b) in zip(modeDiff, maxMode)]
			medianDiff = [abs(a-b) for (a,b) in zip(wMedian, median)]
			maxMedian = [max(a,b) for (a,b) in zip(medianDiff, maxMedian)]
			stdDiff = [abs(a-b) for (a,b) in zip(wStddev, stddev)]
			maxStd = [max(a,b) for (a,b) in zip(stdDiff, maxStd)]
			edgeDiff = [abs(a-b) for (a,b) in zip(wEdge, edge)]
			maxEdge = [max(a,b) for (a,b) in zip(edgeDiff, maxEdge)]
			"""except:
				meanDiff = [0]*3
				modeDiff = [0]*3
				medianDiff = [0]*3
	...
	#normalize
	for (label, meanDiff, modeDiff, medianDiff, stdDiff, edgeDiff) in combined:
		meanDiff = [a/(b or 1) for (a,b) in zip(meanDiff, maxMean)]
		#modeDiff = [a/(b or 1) for (a,b) in zip(modeDiff, maxMode)]
		#medianDiff = [a/(b or 1) for (a,b) in zip(medianDiff, maxMedian)]
		#stdDiff = [a/(b or 1) for (a,b) in zip(stdDiff, maxStd)]
		#edgeDiff = [a/(b or 1) for (a,b) in zip(edgeDiff, maxEdge)]
		combinedNormalized.append((label,meanDiff,modeDiff,medianDiff,stdDiff,edgeDiff))
		if label==1:		
			dataPos.append((label,meanDiff,modeDiff,medianDiff,stdDiff,edgeDiff))
		else:
			dataNeg.append((label,meanDiff,modeDiff,medianDiff,stdDiff,edgeDiff))
	return (dataPos, dataNeg, combinedNormalized)
